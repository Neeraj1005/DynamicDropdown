...
<!-- Use DatePicker for this from https://jqueryui.com/datepicker/#dropdown-month-year -->
					<div class="form-group row">
						<label for="dob" class="col-sm-4 col-form-label text-right">DateOfBirth</label>
						<div class="col-sm-6">
							<input type="text" class="form-control" name="dob" id="datepicker" placeholder="" value="" required>				
						</div>
					</div>
		...
		$(document).on('change','.countrycategory', function(){
			...
			$.ajax({
				...
				success:function(data){
					...
					op+='<option value="0" selected disabled>SelectState</option>';
					for(var i=0;i<data.length;i++){
						op+='<option value="'+data[i].id+'">'+data[i].statecategory+'</option>';
					}
					div.find('.statecategory').html(" ");
					div.find('.statecategory').append(op);
				},
				error:function(){
